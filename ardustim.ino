...
//  ADCSRA |= B01000000;
  ...
  //i=sizeof(dizzy_four_cylinder_friendly_name1);
  //for(w=0;w<i;w++){
   // lcd.print(dizzy_four_cylinder_friendly_name1[w]);
 // }
...

{
  ...
  if(digitalRead(push1)==1){
    cli();
    selected_wheel++;
    if(selected_wheel>=(sizeof(AWAD)/17)){
       selected_wheel=0;
    }
    PORTB=0;
  ...
  //Serial.println(analogRead(A0));
     // Print a message on both lines of the LCD.
      //lcd.setCursor(0,0);   //Set cursor to character 2 on line 
        //rpm=150
        //cli();
            if( analogRead(pot)<153){ 
              //sei();
           // TCCR1B &=~(1 << CS10);  
            lcd.clear();
            wanted_rpm = 150;
           ...
           lcd.print(wanted_rpm);
           lcd.setCursor(0,1);
           lcd.print(AWAD[selected_wheel]);
           //mode = FIXED_RPM;
          //wanted_rpm = 2000;
          //reset_new_OCR1A(wanted_rpm);
          TCCR1B |= (1 << CS10);
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)<153&&digitalRead(push1)==0&&digitalRead(push2)==0);
        }
        //rpm=300   
       else if( analogRead(pot)>=153&&analogRead(pot)<256){
           //TCCR1B &=~(1 << CS10);
           lcd.clear();
            wanted_rpm = 300;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while( analogRead(pot)>=153&&analogRead(pot)<256&&digitalRead(push1)==0&&digitalRead(push2)==0);
      }
           
        //rpm=450
       else if( analogRead(pot)>=256&&analogRead(pot)<358){
       //TCCR1B &=~(1 << CS10);
       lcd.clear();
            wanted_rpm = 450;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while( analogRead(pot)>=256&&analogRead(pot)<358&&digitalRead(push1)==0&&digitalRead(push2)==0);
       }
 //rpm=600
      else if( analogRead(pot)>=358&&analogRead(pot)<416){
      //TCCR1B &=~(1 << CS10);
      lcd.clear();
            wanted_rpm = 600;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
         while( analogRead(pot)>=358&&analogRead(pot)<416&&digitalRead(push1)==0&&digitalRead(push2)==0);
      }
//rpm=750
     else if(analogRead(pot)>=416&&analogRead(pot)<563){
        //    TCCR1B &=~(1 << CS10);
            lcd.clear();
            wanted_rpm = 750;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=416&&analogRead(pot)<563&&digitalRead(push1)==0&&digitalRead(push2)==0);
     }
//rpm=900
     else if(analogRead(pot)>=563&&analogRead(pot)<665){
     //TCCR1B &=~(1 << CS10);
     lcd.clear();
            wanted_rpm = 900;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=563&&analogRead(pot)<665&&digitalRead(push1)==0&&digitalRead(push2)==0);
      }
//rpm=1050
     else if(analogRead(pot)>=665&&analogRead(pot)<768){
     //TCCR1B &=~(1 << CS10);
     lcd.clear();
            wanted_rpm = 1050;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=665&&analogRead(pot)<768&&digitalRead(push1)==0&&digitalRead(push2)==0);
     }
//rbm=1200
    else if(analogRead(pot)>=768&&analogRead(pot)<870){
    //TCCR1B &=~(1 << CS10);
    lcd.clear();
            wanted_rpm = 1200;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=768&&analogRead(pot)<870&&digitalRead(push1)==0&&digitalRead(push2)==0);
      }
//rbm=1350
    else if(analogRead(pot)>=870&&analogRead(pot)<972){
    //TCCR1B &=~(1 << CS10);
    lcd.clear();
            wanted_rpm = 1350;
            lcd.print("RBM: ");
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=870&&analogRead(pot)<972&&digitalRead(push1)==0&&digitalRead(push2)==0);
      }
   ...
   lcd.clear();
          ...
          TCNT1=0;
           setRPM(wanted_rpm);
           while(analogRead(pot)>=972&&analogRead(pot)<=1023&&digitalRead(push1)==0&&digitalRead(push2)==0);                                                    
      }
